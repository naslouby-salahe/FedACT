...
def architecture_shape_violations(repository_root: Path) -> list[str]:
    ...
    violations = [
        f"missing production component: {name}" for name in sorted(REQUIRED_COMPONENTS - present)
    ]
    violations.extend(
        f"unknown production component remains: {name}"
        for name in sorted(present - REQUIRED_COMPONENTS)
    )
    violations.extend(
        f"obsolete production component remains: {name}"
        for name in sorted(present & FORBIDDEN_COMPONENTS)
    )
    ...
    violations.extend(
        f"missing test area: tests/{area}"
        for area in sorted(REQUIRED_TEST_AREAS)
        if not (tests_root / area).is_dir()
    )
...
@pytest.mark.parametrize(
    ...
    [
        "src/fedact/utils.py",
        "src/fedact/core/__init__.py",
        "src/fedact/cli/main.py",
        "tests/quality/test_old.py",
    ],
)
def test_repository_shape_rejects_structural_escape_hatches(
