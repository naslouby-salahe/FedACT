...
from tests.architecture.architecture_rules import module_name, parse_source, production_source_files, relative_source_path
...
PACKAGE_DEPENDENCIES: dict[str, frozenset[str]] = {
    ...
    "fedact.certification": frozenset({"fedact.domain", "fedact.config", "fedact.data", "fedact.learning"}),
    "fedact.experiments": frozenset({"fedact.domain", "fedact.config", "fedact.data", "fedact.learning", "fedact.certification", "fedact.analysis"}),
    "fedact.analysis": frozenset({"fedact.domain", "fedact.config", "fedact.experiments", "fedact.artifacts"}),
    "fedact.artifacts": frozenset({"fedact.domain", "fedact.config"}),
    "fedact.workflow": frozenset({"fedact.domain", "fedact.config", "fedact.data", "fedact.learning", "fedact.certification", "fedact.experiments", "fedact.analysis", "fedact.artifacts"}),
...
def test_internal_dependency_graph_is_fail_closed(repository_root) -> None:
    violations: list[str] = []
    for source in production_source_files(repository_root):
        importer_owner = owner(module_name(repository_root, source))
        if importer_owner in {None, "fedact"}:
            continue
        for imported in imports(parse_source(source)):
            imported_owner = owner(imported)
            if imported_owner not in {None, "fedact", importer_owner} and imported_owner not in PACKAGE_DEPENDENCIES[importer_owner]:
                violations.append(f"{relative_source_path(repository_root, source)}: {importer_owner} may not import {imported_owner}")
