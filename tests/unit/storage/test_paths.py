...
def layout(repository_root: Path) -> WorkspaceLayout:
    ...
    return WorkspaceLayout(
        ...
        workspace=loaded.values.workspace,
    )
...
def test_staging_directory_comes_from_configuration(repository_root: Path) -> None:
    workspace = layout(repository_root)
    assert workspace.staging_directory() == repository_root / "outputs" / "cache" / "staging"
