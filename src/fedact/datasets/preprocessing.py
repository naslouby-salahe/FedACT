...
class PreparationOutcome:
    ...
    def exclusion_count(self, reason: ExclusionReason) -> int:
        return sum(record.excluded_count for record in self.exclusions if record.reason is reason)


def canonical_source_order_key(sample_id: SampleIdentifier) -> str:
    return sample_id
...
def select_low_variance_features(
    ...
    scale_standardization_floor: float,
) -> frozenset[int]:
    ...
    for column in range(dimension):
        ...
        if variance < scale_standardization_floor * scale_standardization_floor:
            low_variance.add(column)
    return frozenset(low_variance)


@dataclass(frozen=True)
class SupportAssessment:
    malicious_support_before: int
    malicious_support_after: int
    control_support_before: int
    control_support_after: int

    def meets_minimum(self, minimum_support_per_class: PositiveInt) -> bool:
        return (
...
def no_adjacent_window_pooling(support_a: int, support_b: int, minimum: PositiveInt) -> bool:
    return support_a >= minimum and support_b >= minimum


def horizon_enters_confirmatory_analysis(availability: HorizonAvailability) -> bool:
