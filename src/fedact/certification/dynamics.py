...
def fit_scalar_model(
    ...
    if len(centers) < 2:
        raise ValueError("scalar dynamics require at least two temporal centers")
    arrs = [np.array(c) if isinstance(c, torch.Tensor) else c for c in centers]
    x = np.stack(arrs[:-1])
    y = np.stack(arrs[1:])
    denom = np.sum(x * x)
    if denom <= 0.0:
        raise ValueError("scalar dynamics require nonzero historical variation")
...
def process_error_radius(
    ...
    if norms.size == 0:
        raise ValueError("process-noise radius requires observed temporal residuals")
    return float(np.quantile(norms, quantile))


def propagate_radius(
    initial_radius: NormValue,
    a_coefficient: CoordinateValue,
    process_noise_radius: NormValue,
    horizon_steps: SampleCount,
) -> NormValue:
    r = initial_radius
    for _unused in range(horizon_steps):
        r = abs(a_coefficient) * r + process_noise_radius
    return r
