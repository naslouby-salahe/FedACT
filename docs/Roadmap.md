...
1. **§27/§29 action-certificate-dependent work (LAMDA + EMBER2024).** Both acquired dataset releases are feature-only; neither contains raw executable bytes. Action generation (problem-space mutation, action-certificate validation, Win32/Win64 identification-and-contrast work) cannot run without them.
   ...
   - Disk budget is also still a real constraint: only ~45GB free vs. 7,892+ malicious samples in the 2023 Baseline release alone (likely 100s of GB of raw APKs if acquired in full) — acquisition must stay staged/bounded, not run against the full corpus in one pass. No experiment-level driver yet iterates real cutoff-eligible sample populations through this pipeline at scale; everything above is verified per-function/per-sample, not yet wired into a `run_*` workflow.
   - **Next concrete step, precisely scoped (not yet implemented):** `run_action_certificate_validation` (`experiments/validation.py:317`) reads `actions.json`, but **no code anywhere in this repo has ever written that file** — `_ActionObservation`'s schema (`sample_id`, `lower_bound`, `upper_bound`, `domain_valid`, `set_diameter`, `historical_diameter_quantile`, `point_score`, `later_real_alignment_score`, ...) was defined speculatively, ahead of real action generation. Closing this loop with real APK-derived actions is a distinct, substantial scientific-wiring task, not a small addendum — traced its exact required math so it doesn't need re-deriving from scratch:
     - `set_diameter` = `box_diameter_bound` (already implemented, `certification/actions.py`) over the coordinate-support bounds of the candidate's feasible set — reuse directly.
     - `point_score` = the "propagated Chebyshev-center point score" (§26.3) = `(lower_bound + upper_bound) / 2` of the action's `ActionInterval` — trivial once the interval exists.
     - `later_real_alignment_score` (§16.2) = \(q_o^\top \hat g^{real}_{c,(T,T+h]}\): the action's normalized displacement direction dotted with the cohort's REAL observed future transition displacement over the horizon after cutoff. The `ĝ^real` term is the same kind of quantity `_embedded_transition_displacement` (`experiments/generalization.py`) already computes for other comparators (embed-space class-mean-before vs. class-mean-after over a window) — reusable, just needs to target the (T, T+h] window for the SAME cohort as the operator's source sample family.
     - `historical_diameter_quantile` (§25.2/config `forecast_set_diameter_abstention.historical_realized_diameter_quantile: 0.90`) = the fixed configured percentile of the **pooled inner-historical** `D_box` distribution (i.e., `box_diameter_bound` computed over historical, pre-cutoff transition replicates for the cutoff — same historical-replicate machinery already used for control-transition replicates elsewhere), using "the §17 linear quantile rule" (check §17 for the exact interpolation method before using `np.percentile`'s default).
     - `leave_one_client_out_passed`: **resolved.** §14.2 (`docs/Roadmap.md:1440`) states LOCO stability is evaluated "When K≥2." LAMDA is fixed as one corpus-level client (K=1, §10.2), so this check is vacuously inapplicable — the `_ActionObservation` schema's own default (`= True`) is correct, not an oversight. `domain_valid` is exactly `CandidateValidityRecord.status is ValidityStatus.VALID` from this session's new 4-layer validity work.
     - **Found the real blocker, not yet resolved: the L_o/U_o interval-construction formula for a query action direction has never been implemented for real data anywhere in this codebase, and reusing what exists risks guessing.** The real client-procedure fit (`identification.py:fit_lamda_client_constraint`, already real and committed) produces a `ClientConstraint` — an *ellipsoid* uncertainty set around a point estimate (`beta`/`uncertainty_radius`, `covariance`, `subspace`, `projector`) — not literal vertices. `certification/actions.py`'s `support_interval(direction, vertices)` is for an explicit vertex set (a different representation) and `certification/uncertainty.py`'s `solve_action_interval`/`solve_support_bounds` don't obviously match §12.2's `L_o = min_{g∈G} q_o^T g` dot-product formula for an ellipsoid `G` (in particular `solve_action_interval` takes `np.linalg.norm` of a vector, which silently discards sign — fine if that function is a synthetic/math-verification-only utility, wrong if reused for a real signed projection). Nothing in the repo currently turns a `ClientConstraint` + a query direction `q_o` into an `[L_o, U_o]` pair — deriving the correct ellipsoid-support-function formula (how `beta`/`covariance`/`projector` combine for an arbitrary direction) from scratch, under time pressure, without an existing tested precedent to check against, is exactly the kind of methodological guess CLAUDE.md prohibits for confirmatory evidence. This needs either explicit roadmap clarification/an existing formula pointer, or a dedicated derivation-and-verification pass (ideally with a synthetic known-truth check, matching how the rest of this project validates its statistical machinery) — not a rushed guess.
     - Given the statistical stakes (this is the FIRST real exercise of the action-certificate confirmatory pathway, §16.2/§25/§26, anywhere in the codebase), the remaining pieces above (`set_diameter`, `point_score`, `later_real_alignment_score`, `historical_diameter_quantile`, LOCO) are ready to wire up; the L_o/U_o ellipsoid-support-function derivation is the one piece still blocking a correct, honest `actions.json`.
