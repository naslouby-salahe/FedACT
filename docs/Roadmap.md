...
1. **§27/§29 action-certificate-dependent work (LAMDA + EMBER2024).** Both acquired dataset releases are feature-only; neither contains raw executable bytes. Action generation (problem-space mutation, action-certificate validation, Win32/Win64 identification-and-contrast work) cannot run without them.
   - **APK path: acquisition and feature re-extraction now real, resolved.** AndroZoo API key acquired and wired (`src/fedact/data/androzoo.py`); verified end-to-end against 3 real LAMDA-hash samples (exact SHA-256 match). A from-scratch LAMDA static-feature re-extractor (`src/fedact/data/lamda_apk_features.py`, androguard-based) independently reproduces 9 of the 10 LAMDA feature categories (4,524 of 4,561 dims, ~99.2%) with **zero mismatches** against real corpus ground truth on all 3 verified samples: `ActivityList`, `BroadcastReceiverList`, `ServiceList`, `RequestedPermissionList`, `IntentFilterList`, `HardwareComponentsList`, `RestrictedApiList`, `SuspiciousApiList`, `URLDomainList`. Key finding: activity/receiver/service names must be read as the *raw, unresolved* `android:name` manifest attribute, not androguard's package-resolved name — the two differ whenever a manifest declares a bare/relative name.
   - **Remaining genuine gap: `UsedPermissionsList` (37 of 4,561 dims, ~0.8%).** Confirmed via direct counter-example (a permission both requested and in-vocabulary, but not flagged "used") that this category requires true permission-to-API reachability analysis, not just requested-permission membership. No permission-to-API mapping (e.g. Axplorer/PScout-style) is pinned by the roadmap or present in this repo, and any such mapping is itself version-specific to an Android API range that would not cover LAMDA's full 2013-2025 span without introducing an unverified, unpinned methodological choice. Left as an honest, documented partial-fidelity gap — these 37 dims are not computed for re-extracted/transformed samples rather than guessed.
   - **§12.4 operator construction: both APK families now real, verified end-to-end.** `src/fedact/data/lamda_apk_mutations.py` implements `permission-neutral-resource-injection` (zip-level append, strips stale signature entries) and `unreachable-benign-gadget-injection` (apktool decompile, add one deterministic smali class, rebuild), both followed by `zipalign`+`apksigner` re-signing against a deterministic project-pinned debug keystore. Verified on a real downloaded APK: signature validates (v1/v2/v3), androguard re-parses the result, and package/permissions/activities are byte-for-byte identical to the source in both cases (only the gadget/resource is added). `src/fedact/certification/actions.py` gained `apk_structural_validity_of` (androguard as primary parser, `aapt2 dump badging` as an independent secondary parser, cross-checked package identity) and `maliciousness_validity_of` (real `clamscan` invocation) to match the existing PE-side `StructuralValidity`/`MaliciousnessValidity` machinery; `StructuralValidity.expected_machine_type` was renamed to `expected_format_identity` since the field is genuinely domain-general (PE and APK) not PE-specific.
   - **Real environment finding:** ClamAV's stock signature database does **not** detect the 2 real malicious LAMDA/AndroZoo samples tested so far (`clamscan` returns clean). Per the roadmap's own rule this makes those specific samples `MALICIOUSNESS_VALIDATION_UNAVAILABLE`, not evidence of a broken pipeline — but it means the eventual confirmatory sample count usable for the maliciousness-preservation layer may be a real subset of the corpus, not all of it. Not yet measured at scale.
   - **Fixed a real correctness bug this finding exposed:** `CandidateValidityRecord.status` was collapsing `MALICIOUSNESS_VALIDATION_UNAVAILABLE` (source never flagged) into the same `INVALID` verdict as "the mutation broke detection" — contradicting this roadmap's own line above. Given ClamAV's measured low detection rate on this corpus, that bug would have silently mislabeled most real candidates as broken mutations. Added the missing third `ValidityStatus` value and verified all three cases directly.
   - **Improved real detection coverage, still not enough for the 2 tested samples:** added `src/fedact/data/clamav_signatures.py`, which acquires a small set of real, MIT-licensed, malware-focused ClamAV community signature databases (Sanesecurity `malwarehash.hsb`/`rogue.hdb`/`foxhole_filename.cdb`/`foxhole_generic.cdb` — hash/CDB-based, not the larger pattern-matching NDB sets, to keep acquisition small and scans fast) and records their SHA-256 + fetch timestamp in a manifest, matching the roadmap's "file hashes and publication timestamp are recorded" requirement. `maliciousness_validity_of` now scans with `-d <official> -d <supplementary>` when a supplementary directory is supplied, and `run_lamda_action_generation` acquires and passes it automatically. Still does not detect the 2 real samples tested — a genuine, unresolved coverage gap in the freely available signature data, not a code defect.
   - **Real environment finding:** Java tools (`apksigner`/`apktool`/`keytool`) can block for many minutes on this host's default `SecureRandom` source; fixed by forcing `-Djava.security.egd=file:/dev/./urandom` via `_JAVA_OPTIONS` in every such subprocess call (`java_subprocess_environment()` in `lamda_apk_mutations.py`). This host was also observed under severe unrelated CPU contention (load average 21 on 10 cores, from an unrelated project's background jobs) during this work, which independently degrades wall-clock time for any CPU-heavy step — worth ruling out first if future runs here look "hung."
   - **§12.5 dynamic validation: real emulator harness built and verified end-to-end.** `src/fedact/data/lamda_apk_emulator.py` boots a headless AVD (Android 30, x86_64, `emulator -no-window -gpu swiftshader_indirect`), installs source/transformed APKs via `adb`, launches via `monkey -c android.intent.category.LAUNCHER`, runs a seeded `monkey` event stream, and parses `logcat` for crash/ANR patterns and an observable event set (activity starts, `Displayed` lines) to compute the Jaccard similarity §12.5 requires. `apk_dynamic_validity_of` in `certification/actions.py` wires this into real `ExecutionSmokeValidity`/`BehaviorValidity` records. Verified against a real AndroZoo APK and its resource-injection mutation: both installed and launched successfully; the real (not fabricated) result on this one run was `no_new_crash_or_anr=False` and `jaccard_similarity=0.25` (below the configured 0.80 floor) — a genuine negative result for this sample/config, left as-is rather than tuned toward a pass.
     - Fixed `ExecutionSmokeValidity.is_valid`, which hardcoded a `<= 30.0` comparison instead of using the governed `operators.validation.execution_timeout_seconds` (60s) — added a `configured_timeout_seconds` field and compare against that instead.
     - **Real environment finding:** this host's user was not in the `kvm` group, so the emulator refused to start (`x86_64 emulation currently requires hardware acceleration!`). `usermod -aG kvm` was applied, but a running shell session needs a fresh login (or `sg kvm -c '...'`) to pick up new group membership — confirmed the fix works via `sg kvm -c`, but the production code does not (and should not) silently work around missing group membership; whoever runs this pipeline needs a session with real `kvm` group access already established.
   - Disk budget is also still a real constraint: only ~45GB free vs. 7,892+ malicious samples in the 2023 Baseline release alone (likely 100s of GB of raw APKs if acquired in full) — acquisition must stay staged/bounded, not run against the full corpus in one pass.
   - **`run_lamda_action_generation` is now invoked by the `ACTION_CERTIFICATE_VALIDATION` CLI workflow** (`workflow.py`), which boots the emulator (via `ANDROID_SDK_ROOT`, skipping cleanly with a clear message if unset or no APKs acquired), runs generation, shuts it down, then proceeds to `run_action_certificate_validation` as before. The emulator system image is a governed config value (`operators.validation.android_system_image`). Package names are derived automatically from each acquired APK (no external mapping needed).
   - **`actions.json` is now written for real, for the first time anywhere in this codebase** (`experiments/lamda_action_generation.py:run_lamda_action_generation`), consumed by the already-existing `run_action_certificate_validation` (`experiments/validation.py:317`). Resolved the interval-construction question that blocked this: `tests/scientific/test_feasible_set_contracts.py` (an existing ground-truth test) confirms the feasible set for K=1 is an isotropic `L2Ball` of radius `beta` — `certificate.py`'s `FeasibleSet.diameter = 2 * sum(radii)` treats it exactly that way — not a general ellipsoid using `covariance` as a shape operator. For a unit query direction `q_o`, the ball's support function collapses to the simple, defensible closed form `L_o = q_o·ĝ - beta`, `U_o = q_o·ĝ + beta`, `D_box = 2*beta`. `point_score` is the interval's midpoint; `later_real_alignment_score` uses the real (T, T+h] realized cohort transition (via `windowed_mean`); `historical_diameter_quantile` pools `2*beta` over nested inner-historical refits of `fit_lamda_client_constraint` at the configured percentile; `leave_one_client_out_passed` defaults `True` since LAMDA's K=1 makes LOCO vacuously inapplicable (§14.2, `docs/Roadmap.md:1440`).
   - Verified end-to-end on one real AndroZoo-acquired APK against a real booted emulator: ran to completion with no exceptions, wrote a valid (honestly empty, single-sample) `actions.json` — `INSUFFICIENT_EVIDENCE` was the correct, unforced outcome given only one acquired sample.
   - **Acquisition scaled up under an explicit user disk budget ("a couple GB is fine, hundreds is not").** Added `acquire_lamda_apks_within_budget` (`src/fedact/data/androzoo.py`) — stops once the newly-downloaded total hits a caller-supplied byte cap, already-cached files don't count against it. Used to acquire 59 real APKs (2.4GB) spanning the dominant cohort's full 2023 chronology (previously just 1 sample).
   - **`actions.json` is now written incrementally, not only at the end of the run.** A full run trains a representation encoder and refits several nested client constraints per eligible cutoff, then runs a real emulator round-trip per candidate — genuinely long-running under this host's sustained CPU contention. The previous end-of-run-only write meant an interrupted run lost all progress; `run_lamda_action_generation` now persists (atomically) after every action.
   - **Ran to completion against all 59 real acquired samples: 685s, real, honest `INSUFFICIENT_EVIDENCE`.** All 3 candidate cutoffs in the eligible range were rejected by the real fitting pipeline before reaching candidate generation (`candidates_considered=0`) — most likely real data sparsity in this cohort's month-by-month support (only Nov 2023 and Dec 2023 have substantial counts; most other months have 1-4 samples), which is exactly the kind of frequent, legitimate abstention already documented as normal for sparse real data elsewhere in this project (`run_lamda_identification_diagnostics` abstains on many cutoffs for the same reason). Not forced to a pass. More acquisition volume concentrated in the sparser months, or a different cohort, would be the next real lever — not more code.
   - **Found and fixed a real performance hotspot, not a scientific one.** `_apk_package_name` (action generation) and `apk_structural_validity_of` (certification) both called `AnalyzeAPK`, which builds a full DEX cross-reference/method-graph `Analysis` object, just to read `apk.get_package()` from the manifest. Neither ever touched the DEX or analysis result. Switched both to `APK(...)` (manifest-only, no DEX parsing) — same value, same behavior, no scientific change. This alone cut the 59-sample diagnostic run from 685s to 9.8s.
   - **Found and fixed the real root cause of the universal `INSUFFICIENT_EVIDENCE`: it was a dataset-scope bug, not real sparsity.** `raw_root` was hardcoded to `Baseline/2023` across `identification.py`, `lamda_population.py`, and `lamda_action_generation.py` — an arbitrary single-calendar-year slice. LAMDA's own dataset card describes it as a 2013–2025 longitudinal corpus built specifically for concept-drift analysis, and this project's own temporal protocol (`historical_training_window_months: 12`) cannot function correctly on one year of data. Restricting to 2023 alone starved every family's malicious-support windows regardless of family choice — confirmed by rerunning `run_lamda_identification_diagnostics` (unrelated, pre-existing code) against the same 2023-only scope: `cutoffs_fitted=0` there too, for the same reason. The unmodified `minimum_support_per_class: 200` threshold was never the problem. Fixed by making `load_lamda_records` glob recursively (`data/lamda.py`) and pointing all `raw_root` sites at `Baseline` instead of `Baseline/2023` — no threshold, formula, or statistical rule changed. Against the real full corpus, the real dominant cohort is `dowgin` (32,475 malicious records, 2013–2023) with 19 real endpoints now passing the same unmodified 200-per-side floor, concentrated 2014-02 through 2014-08. Re-acquiring real AndroZoo APKs for this cohort/window (the prior `smsreg`/2023-only 59-sample acquisition is now scientifically superseded, not deleted) to actually reach candidate generation.
   - **PE path does not exist.** The roadmap names no authorized acquisition route for raw PE bytes matching the EMBER2024 release (`docs/Roadmap.md:1726`); EMBER releases are feature-only by design. Directly confirmed: the acquired 101GB EMBER2024 corpus (`data/raw/EMBER2024`, 449 weekly JSONL files) contains only derived fields (`histogram`, `byteentropy`, `strings`, `header`, `section`, `imports`, `exports`, hashes, metadata) — no embedded binary. This blocker is permanent absent a new dataset decision, not a pending acquisition. §29's Win32/Win64 work and the §14.1 baseline `generic-robust-no-certificate` therefore stay blocked regardless of the AndroZoo unblock above.
   - **Found and fixed a real spec/implementation mismatch in §31 cross-corpus generalization.** `run_cross_corpus_generalization` loaded a "locked transfer manifest" (source encoder/detector checkpoint + a fitted feature adapter) and scored EMBER2024 samples with a LAMDA-trained detector — a transfer-learning claim. Nothing in the codebase ever wrote that manifest, so the path never actually ran; and §31 itself says a detector-performance win alone is insufficient for a generalization claim, and instead specifies applying the *same independent* FedACT pipeline to both corpora and comparing whether the mechanism replicates. `run_ember2024_identification_diagnostics` already existed as exactly that independent EMBER2024 pipeline (mirroring `identification.py`'s LAMDA flow, itself already correct) but was never compared against LAMDA's results. Replaced the transfer-scoring body with one that reads both corpora's persisted `identification-diagnostics.json` artifacts and reports whether the control-based malicious-transition mechanism independently fits in both (`mechanism_replicates`). Since EMBER2024 problem-space PE actions are `operator_ineligible` (per the PE-bytes blocker above), paired action-interval evidence is honestly `0`, giving `INSUFFICIENT_EVIDENCE` via the existing (previously unused) `confirmatory_outcome_for_cutoffs` rule for the confirmatory claim — while the descriptive mechanism-replication signal is still reported separately, exactly matching §31's own explicit fallback for missing paired cutoffs. Removed the dead `_TransferManifest`/`_FeatureAdapter`/`_load_feature_adapter`/`_workspace_path`. De-privatized `IdentificationDiagnosticsArtifact` (LAMDA) and `Ember2024IdentificationDiagnosticsArtifact` for this cross-module read. Also found and deleted one stale pre-fix `identification-diagnostics.json` (cohort `smsreg`, from before the §12 dataset-scope fix above) that would otherwise have been silently read as valid evidence. A real LAMDA identification run (multi-hour at full-corpus scale) and an EMBER2024 identification run are still needed to produce actual comparison numbers; not run yet to avoid competing with the in-progress action-generation run on this host.
