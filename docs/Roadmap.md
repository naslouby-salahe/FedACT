...
1. **§27/§29 action-certificate-dependent work (LAMDA + EMBER2024).** Both acquired dataset releases are feature-only; neither contains raw executable bytes. Action generation (problem-space mutation, action-certificate validation, Win32/Win64 identification-and-contrast work) cannot run without them.
   ...
   - **Remaining before action-certificate evidence is real:** (a) disk budget — only ~48GB free in this environment vs. 7,892+ malicious samples in the 2023 Baseline release alone (likely 100s of GB of raw APKs); acquisition must be staged/bounded, not run against the full corpus in one pass; (b) §12.4's actual operator construction (rebuild + re-sign a modified APK) needs an APK build/sign toolchain (apktool/apksigner/zipalign) — not yet installed/attempted; (c) §12.5's 4-layer validation needs a project-pinned Android emulator image + ADB monkey-testing harness and ClamAV with a pinned signature snapshot — neither installed nor feasibility-tested (headless emulator boot under WSL2/`/dev/kvm` is unverified).
