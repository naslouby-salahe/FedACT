...
1. **§27/§29 action-certificate-dependent work (LAMDA + EMBER2024).** Both acquired dataset releases are feature-only; neither contains raw executable bytes. Action generation (problem-space mutation, action-certificate validation, Win32/Win64 identification-and-contrast work) cannot run without them.
   ...
   - **`actions.json` is now written incrementally, not only at the end of the run.** A full run trains a representation encoder and refits several nested client constraints per eligible cutoff (only 3 cutoffs are eligible for the current 59-sample acquisition, but each cutoff's encoder training alone is a real ~54K-row/4561-dim training job), then runs a real emulator round-trip per candidate — genuinely long-running under this host's sustained CPU contention. The previous end-of-run-only write meant an interrupted run lost all progress; `run_lamda_action_generation` now persists (atomically) after every action, so a real 59-sample run is in progress as a detached background process rather than one blocking foreground call, and will keep whatever it's produced so far even if interrupted.
