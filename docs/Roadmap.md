...
1. **§27/§29 action-certificate-dependent work (LAMDA + EMBER2024).** Both acquired dataset releases are feature-only; neither contains raw executable bytes. Action generation (problem-space mutation, action-certificate validation, Win32/Win64 identification-and-contrast work) cannot run without them.
   ...
   - **Still not done:** §12.5's dynamic layers (execution-smoke + behavior-preservation) need a real Android emulator + ADB monkey-testing harness. Android SDK cmdline-tools were downloaded and `sdkmanager`/`avdmanager` are present (needs JDK 17, already installed, as `JAVA_HOME`), but no system image has been fetched or AVD booted yet — headless emulator feasibility on this host (KVM device present, but under the CPU contention noted above) is still unverified. Disk budget is also still a real constraint: only ~48GB free vs. 7,892+ malicious samples in the 2023 Baseline release alone (likely 100s of GB of raw APKs if acquired in full) — acquisition must stay staged/bounded, not run against the full corpus in one pass.
